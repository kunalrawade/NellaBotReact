...
import {applicationContextPath} from './api'
/////

/////D:\NellaBot\NellaDEV_UI\src\App.js
...
class MainBot extends React.Component {
	...
	getBotConfig = () =>{
		debugger
		var data1 = {}
		if(this.state.port=="http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:9000"){
			data1.flag = "kapitxbot"
		 }
		 else if(this.state.port=="http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8000"){
			data1.flag = "pension"
		 }
		 else if(this.state.port=="http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:8003"){
			data1.flag = "telecombot"
		 }
		 else if(this.state.port=="http://ec2-18-219-146-105.us-east-2.compute.amazonaws.com:8002"){
			data1.flag = "hrbot"
		 }
		...
		else if(this.state.port=="http://ec2-18-219-146-105.us-east-2.compute.amazonaws.com:8008"){
		...
		else if(this.state.port=="http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8022"){
	   data1.flag = "nellinfotech"
		}
		else if(this.state.port=="http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8023"){
	   data1.flag = "salesforce"
		}
		else if(this.state.port=="http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:8024"){
	   data1.flag = "nbfc"
		}
		
		axios.post(applicationContextPath+':8080/NellaConsole-Services/getBotConfig', data1,{
        }).then(response => {
	...
	pensionBot = ()=>{
		...
		loginDataObj.username = this.state.LoginData.username
		loginDataObj.password = this.state.LoginData.password
			axios.post('http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8000/faq/mob/', loginDataObj, {
			}).then(response => {
				this.setState({
					loginDataObj : response.data,
					authentication : true
				...
				var port;
				if(response.data.authentication == "success"){
					 port = "http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8000"		
				}
				this.setState({botflag : true, port:port,mainFlag:true})
				this.getBotConfig()	
				this.autocomplete();
	...
	telecomBot = ()=>{
		...
		loginDataObj.password = this.state.LoginData.password
			...
			axios.post('http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:8003/faq/mob/', loginDataObj, {
			}).then(response => {
				...
				var port;
				if(response.data.authentication == "success"){
					port = "http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:8003"
			//		this.props.history.push("/mainBot",port)
	...
	hrBot = ()=>{
		...
		loginDataObj.password = this.state.LoginData.password
			axios.post('http://ec2-18-219-146-105.us-east-2.compute.amazonaws.com:8002/faq/mob/', loginDataObj, {
				...
				var port ;
				if(response.data.authentication == "success"){
					 port = "http://ec2-18-219-146-105.us-east-2.compute.amazonaws.com:8002"
				//	this.props.history.push("/mainBot",port)
	...
	hospitalBot = ()=>{
		...
		loginDataObj.username = this.state.LoginData.username
		loginDataObj.password = this.state.LoginData.password
			axios.post('http://ec2-18-219-146-105.us-east-2.compute.amazonaws.com:8008/faq/mob/', loginDataObj, {
			}).then(response => {
				this.setState({
					loginDataObj : response.data,
					authentication : true
				...
				var port ;
				if(response.data.authentication == "success"){
					port = "http://ec2-18-219-146-105.us-east-2.compute.amazonaws.com:8008"
				//	this.props.history.push("/mainBot",port)
	...
	nellinfotechbot = () => {
		...
		loginDataObj.username = this.state.LoginData.username
		loginDataObj.password = this.state.LoginData.password
			axios.post('http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8022/faq/mob/', loginDataObj, {
			}).then(response => {
				this.setState({
					loginDataObj : response.data,
					authentication : true
				...
				var port;
				if(response.data.authentication == "success"){
					port = "http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8022"
				//	this.props.history.push("/mainBot",port)
				...
				this.autocomplete();
				this.getBotResponceSuccess(this.state.loginDataObj);
				this.onLoginSuccess("nellinfotech")	
				this.getNotification("nellinfotech");
	...
	salesforcebot = () => {
		...
		loginDataObj.username = this.state.LoginData.username
		loginDataObj.password = this.state.LoginData.password
			axios.post('http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8023/faq/mob/', loginDataObj, {
			}).then(response => {
				this.setState({
					loginDataObj : response.data,
					authentication : true
				...
				var port;
				if(response.data.authentication == "success"){
					port = "http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8023"
					this.setState({botflag : true,port:port,mainFlag:true})	
				}
				this.setState({
					loginData : response.data,
				...
				this.autocomplete();
				this.getBotResponceSuccess(this.state.loginDataObj);
				this.onLoginSuccess("salesforce")	
				this.getNotification("salesforce");
				console.log("getBotResponce", response.data)
			}).catch(error => {
				console.log(error);
			});
	...
	nbfc = () => {
		...
		loginDataObj.username = this.state.LoginData.username
		loginDataObj.password = this.state.LoginData.password
			axios.post('http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:8024/faq/mob/', loginDataObj, {
			}).then(response => {
				this.setState({
					loginDataObj : response.data,
					authentication : true
				...
				var port;
				if(response.data.authentication == "success"){
					port = "http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:8024"
					this.setState({botflag : true,port:port,mainFlag:true})	
				}
				this.setState({
					loginData : response.data,
				...
				this.autocomplete();
				this.getBotResponceSuccess(this.state.loginDataObj);
				this.onLoginSuccess("nbfc")	
				this.getNotification("nbfc");
				console.log("getBotResponce", response.data)
			}).catch(error => {
				console.log(error);
			});
...
getItemValue(item){
	...
	getDashBoardData = () =>{
				 ...
				 if(this.state.port=="http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:9000"){
					...
					failureRecords.flag = "nellinfotech"
				}
				else if(this.state.port=="http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8023"){
					failureRecords.flag = "salesforce"
				}
				else if(this.state.port=="http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:8024"){
					failureRecords.flag = "nbfc"
		     }
 
		axios.post(applicationContextPath+':8080/NellaConsole-Services/getDashboardData',failureRecords)
		.then(response => {
	...
	getNotification=(botType)=>{
		...
		axios.post(applicationContextPath+':8080/LiveAgent/getNotificationByUserId',notificationRequest)
		.then(response => {
			debugger
...
handleEmojiPicker = () =>{
	...
	onLoginSuccess=()=>{
		...
		if(this.state.port=="http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:9000"){
			...
			newLoginObj.botType = "nellinfotech"
			}
			else if(this.state.port=="http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8023"){
			newLoginObj.botType = "salesforce"
			}
			else if(this.state.port=="http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:8024"){
				newLoginObj.botType = "nbfc"
		 }
				axios.post(applicationContextPath+':8080/LiveAgent/activeUsers', newLoginObj,{
		}).then(response => {
	...
	showFieldValue = (id) =>{
		debugger
		if(this.state.idname == id){
			this.setState({showFieldValue:false,idname:''})
		}else{
		this.setState({showFieldValue:true,idname:id})
		}
	}
	...
	onSubmitClick = (data) => {
      ...
      this.getDashBoardData()
		...
		axios.post(applicationContextPath+':8080/LiveAgent/userMessage', utteranceObj, {
	...
	isUserWantToChatWithLiveAgent(){
		...
		if(this.state.port=="http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:9000"){
			...
			utteranceObj.botType = "nellinfotech"
		}
		else if(this.state.port=="http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8023"){
			utteranceObj.botType = "salesforce"
		}
		else if(this.state.port=="http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:8024"){
			utteranceObj.botType = "nbfc"
		 }
		 //utteranceObj.message = data
		 utteranceObj.Session_key = login.session_key
		 utteranceObj.Agent_Id = ""
		 utteranceObj.User_Id = login.username
		 //utteranceObj.botType=this.state.port ? this.state.port : "";
		axios.post(applicationContextPath+':8080/LiveAgent/userStatus', utteranceObj, {
	...
	uploadFile = () =>{
	 ...
	 if(this.state.port=="http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:9000"){
		data1.flag = "kapitxbot"
		data.append('botType',"kapitxbot")
	 }
	 else if(this.state.port=="http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8000"){
		data1.flag = "pension"
		data.append('botType',"pension")
	 }
	 else if(this.state.port=="http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:8003"){
		data1.flag = "telecombot"
		data.append('botType',"telecombot")
	 }
	 else if(this.state.port=="http://ec2-18-219-146-105.us-east-2.compute.amazonaws.com:8002"){
		data1.flag = "hrbot"
		data.append('botType',"hrbot")
	 }
	 else if(this.state.port=="http://ec2-18-188-243-169.us-east-2.compute.amazonaws.com:8006"){
		data1.flag = "germanbot"
		data.append('botType',"germanbot")
	 }
	 else if(this.state.port=="http://ec2-18-188-243-169.us-east-2.compute.amazonaws.com:8014"){
		data1.flag = "farmerbot"
		data.append('botType',"farmerbot")
  }
 else if(this.state.port=="http://ec2-18-219-146-105.us-east-2.compute.amazonaws.com:8008"){
	data1.flag = "hospitalbot"
	data.append('botType',"hospitalbot")
  }
  else if(this.state.port=="http://ec2-18-188-243-169.us-east-2.compute.amazonaws.com:8013"){
	data1.flag = "hindibot"
	data.append('botType',"hindibot")
}
else if(this.state.port=="http://ec2-18-188-243-169.us-east-2.compute.amazonaws.com:8004"){
	data1.flag = "interviewBot"
	data.append('botType',"interviewBot")
}
else if(this.state.port=="http://ec2-18-188-243-169.us-east-2.compute.amazonaws.com:8016"){
	data1.flag = "apricot"
	data.append('botType',"apricot")
}
else if(this.state.port=="http://ec2-18-188-243-169.us-east-2.compute.amazonaws.com:8018"){
	data1.flag = "tricontes"
	data.append('botType',"tricontes")
}
else if(this.state.port=="http://ec2-18-188-243-169.us-east-2.compute.amazonaws.com:8021"){
	data1.flag = "germantricontes"
	data.append('botType',"germantricontes")
}
else if(this.state.port=="http://ec2-18-188-243-169.us-east-2.compute.amazonaws.com:8017"){
	data1.flag = "doctorassistant"
	data.append('botType',"doctorassistant")
}
else if(this.state.port=="http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8022"){
	data1.flag = "nellinfotech"
	data.append('botType',"nellinfotech")
}
else if(this.state.port=="http://ec2-18-218-142-38.us-east-2.compute.amazonaws.com:8023"){
	data1.flag = "salesforce"
	data.append('botType',"salesforce")
}
else if(this.state.port=="http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:8024"){
	data1.flag = "nbfc"
	data.append('botType',"nbfc")
		 }
document.getElementById("loader-wrapper1").style.visibility = "visible";
		axios.post(applicationContextPath+':8080/NellaConsole-Services/uplaodDoc', data,{
		}).then(response => {
			document.getElementById("loader-wrapper1").style.visibility = "hidden";
			this.onSubmitClick(response.data.path)
			data1.path = response.data.path
			data1.upload_file_name = response.data.upload_file_name
			return axios.post(applicationContextPath+':8080/NellaConsole-Services/saveuploadedDoc',data1);
		}).then((res) => {
		var today = new Date();
		var h = today.getHours();
		var m = today.getMinutes();
...
document.getElementById("loader-wrapper1").style.visibility = "visible";
		axios.post(applicationContextPath+':8080/NellaConsole-Services/uplaodDoc', data,{
			...
			var infoArea1 = document.getElementById( 'file-upload-filename2' );
		infoArea1.textContent = "Click on submit button to submit form";
	...
	var fileName = this.state.fileList.name
		...
		// var infoArea1 = document.getElementById( 'file-upload-filename2' );
		// infoArea1.textContent = "Please click on upload button to upload file";
  ...
  saveWord=(word)=>{
	...
	render() {
		...
		// 		$('#myList .wrapperdata:hidden').slice(0, 2).show();
		// 		if ($('#myList .wrapperdata').length == $('#myList .wrapperdata:visible').length) {
		// 			$('.readmore').hide();
		// 		}
		// 	});
		// })
...
// 	}
		...
		const notify = this.state.items.map((item,index) => {
			return (
				...
				// <div class="carousel-inner carouselImage" role="listbox">
				// 	{item.notificationType == "image" ?
				// 	<div classNam="carousel-item active">
				// 	<img src={item.notificationMessage} alt="Los Angeles"/>
				// 	</div>:
				// 	item.notificationType == "video" ?
				// 	<div class="carousel-item">
				// 	<object data={item.notificationMessage} />
				// 	</div>:
				// 	// item.notificationType == "audio" ?
				// 	// <div class="carousel-item">
				// 	// <object data={item.notificationMessage}/>
				// 	// </div>:
				// 	<div class="carousel-item">
				...
				// <a class="carousel-control-prev" href="#carousel-example-generic" href="button" data-slide="prev">
				// <span class="carousel-control-prev-icon"></span>
				// </a>
				// <a class="carousel-control-next" href="#carousel-example-generic" href="button" data-slide="next">
				...
				>
                 ...
                 <div className="slide-container" id="myList">
					 {item.notificationMessage ?
					 <div className="wrapper wrapperdata">
					 {item.notificationMessage}
				    </div>:""}
				      {item.notificationImageLink != null && item.notificationImageLink!="" ?
						<div className="wrapper wrapperdata">
							<img style={{width:"85%"}} src={item.notificationImageLink}></img>
						</div>:""}
						{item.notificationVideoLink != null && item.notificationVideoLink  !=""?
							<div className="wrapper wrapperdata">
							<object style={{height:"100%",width:"85%"}} data={item.notificationVideoLink}></object>
						</div>:""}
						{item.notificationAudioLink !=null && item.notificationAudioLink  !="" ?
						<div className="wrapper wrapperdata">
						<audio style={{width:"70%" , marginRight: "50px", marginBottom: "25px"}} controls src={item.notificationAudioLink} />
						</div>:""}
						{item.replay_task !=undefined && item.replay_task!= "" && item.replay_task!= null ?
						<div className="wrapper wrapperdata">		
						 <button type='button' id="" className="btn-outline" onClick={() => this.onNotificationClick("#redirect-"+item.replay_task, item.notificationMessage)} >Select</button>
						</div>:""}
					</div>
...
console.log("value..............................................",value)
				return (
					...
					//   />
							...
							{value.type === "list" && this.state.port != "http://ec2-3-19-215-125.us-east-2.compute.amazonaws.com:9000" 
		...
		return (
			...
			<div>
			<div className="login">
			<SockJsClient url= {applicationContextPath+':8080/LiveAgent/chat-web-socket'} topics={['/topic/agent']}
            onMessage={(msg) => { this.onAgentMessage(msg) }}
            ref={ (client) => { this.clientRef = client }} />
			<SockJsClient url={applicationContextPath+':8080/LiveAgent/chat-web-socket'} topics={['/topic/agentInterfare']}
            onMessage={(msg) => { this.onAgentWantToTalk(msg) }}
            ref={ (client) => { this.clientRef = client }} />
			<SockJsClient url={applicationContextPath+':8080/LiveAgent/chat-web-socket'} topics={['/topic/notification']}
			  ...
			  {/* {this.state.importIcCardModel == true ?
				...
				<div className="chat hide">
					{/* <div className="row justify-content-center h-100"> */}
								...
								<div style={{backgroundColor:this.state.headerColor}} className="card-header msg_head">
									<div className="d-flex bd-highlight">
										...
										<div className="img_cont">
											<img src={logo2} className="rounded-circle user_img" alt="face" />
						   ...
						   {this.state.bodyColor != null && this.state.bodyColor != "" ?
								...
								<div className="d-flex justify-content-start mb-4">
									...
									<img src={logo} className="rounded-circle user_img_msg" alt="face" />
										...
										<div className="msg_cotainer msgnotify" key={'atterenceResponce' + -1}>
											<Carousel
											activeIndex={this.state.activeIndex}
											next={this.next}
											previous={this.previous}
						...
						<div className="col-md-4 text-center">
							<div className="hr_btn">
								<button onClick={() =>this.nellinfotechbot()}>Nell Infotech </button>
							</div>
						</div>
						<div className="col-md-4 text-center">
							<div className="hr_btn">
								<button onClick={() =>this.salesforcebot()}>SalesForce Bot </button>
							</div>
						</div>
						<div className="col-md-4 text-center">
							<div className="hr_btn">
								<button onClick={() =>this.nbfc()}>NBFC</button>
							</div>
						</div>
					</div>
					}
							   ...
							   <div className="shareholder">
											<div>
											   {this.state.intCardData.map((data,idx)=>(
												...
												<i title={data.Entity_Question} onClick={()=>this.showFieldValue(data.Entity_Name)} class="fa fa-info-circle" aria-hidden="true"></i>
												<select style={{marginLeft:'10px'}} title={data.Entity_Question} className="inputName" id={"dropdownvalue"+idx}>
												...
												{
													(this.state.showFieldValue == true && (this.state.idname == data.Entity_Name)) &&
													<span style={{color:"#147de2"}}>{data.Entity_Question}</span>
												}
												<br/><br/></tr>
												...
												//  data.Entity_Name == "from_date" ?
												 <tr className="inputName" title={data.Entity_Question}>
													<td style={{display:"inline-flex",width: '-webkit-fill-available'}}>
												 <i title={data.Entity_Question} onClick={()=>this.showFieldValue(data.Entity_Name)} class="fa fa-info-circle" aria-hidden="true"></i>
												 <input style={{marginLeft:'10px'}} type="date" name="bday" className={data.Entity_Name} onChange={(e)=>this.fromchangeDate(e)}></input>
												 </td> <span className="errormesage">{data.Error}</span>
												 {
													(this.state.showFieldValue == true && (this.state.idname == data.Entity_Name)) &&
													<span style={{color:"#147de2"}}>{data.Entity_Question}</span>
												}
											  ...
											  <br/><br/> </tr>:
											   data.Entity_categoty == "upload_doc" || data.Entity_Name== "upload_doc"?
											  <tr className="inputName" title={data.Entity_Question}>
												   
												 <div className="image-upload inline">
												 <td style={{width: '-webkit-fill-available'}}>
													 <i style={{marginRight:'10px'}} title={data.Entity_Question} onClick={()=>this.showFieldValue(data.Entity_Name)} class="fa fa-info-circle" aria-hidden="true"></i>
													<input style={{marginLeft:'10px'}} type="file" id={data.Entity_Name}  name="myFile" onChange={(e) => this.handleFileSelect1(e)} />
													...
													<div style={{color:"#147de2"}} id="file-upload-filename2"></div>
													</td>
													</div>
											  ...
											  <tr>
												  <td style={{display:"inline-flex", width: '-webkit-fill-available'}}>
												   <i title={data.Entity_Question} class="fa fa-info-circle" onClick={()=>this.showFieldValue(data.Entity_Name)} aria-hidden="true"></i>
												<input style={{marginLeft:'10px'}} type="text" title={data.Entity_Question} className="inputName" id={data.Entity_Name} placeholder={data.Entity_Question} />
												</td>
												<span className="errormesage">{data.Error}</span>
												{
													(this.state.showFieldValue == true && (this.state.idname == data.Entity_Name)) &&
                   ...
                   {this.state.emojipicker == true && 
							...
							{this.state.footerColor != null && this.state.footerColor != "" && this.state.mainFlag?
								...
								<div style={{backgroundColor:this.state.footerColor}} className="card-footer">
